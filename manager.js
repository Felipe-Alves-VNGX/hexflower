/**
 * Hex Flower Manager
 * Use this macro to Import, Edit, or Delete Hex Flowers stored in the World.
 */
...
const CSS = `
<style>
    .hex-flower-dialog .window-content {
        background: #1a1a1a;
        color: #e0e0e0;
        padding: 0;
    }
    .hex-manager-container {
        padding: 10px;
    }
    .hex-manager-table { 
        width: 100%; 
    ...
    .hex-manager-table td {
        padding: 8px 10px;
        border-bottom: 1px solid #333;
    }
    .hex-manager-table tr:last-child td {
        border-bottom: none;
    }
    .hex-manager-table tr:hover {
        background: #2a2a2a;
    }
    .hex-btn {
        background: #333;
    ...
    /* Form Elements */
    input.hex-input, textarea.hex-input {
        background: #222;
        border: 1px solid #444;
        color: #ddd;
        padding: 5px;
    ...
    input.hex-input:focus, textarea.hex-input:focus {
        border-color: #6699ff;
        outline: none;
    }
    textarea.hex-input {
...
async function showManagerDialog() {
    ...
    new Dialog({
        ...
        render: (html) => {
            // Apply custom Dialog class for specific styling if needed
            ...
            save: {
                ...
                callback: async (html) => {
                    ...
                    try {
                        ...
                        showManagerDialog(); // Go back to list

                    } catch (e) {
                        ui.notifications.error("JSON Parse Error: " + e.message);
                        console.error(e);
                        // Prevent closing?
                        // Foundry dialog structure makes preventing close hard in standard buttons callback without creating a new dialog.
                        // We will just let it close and user has to reopen (or we re-open this dialog).
                        // Better UX: Re-open edit dialog with content? Too complex for macro. 
                        // Just warn user.
                    }
                }
            },
