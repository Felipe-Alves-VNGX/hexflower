import { generateSVG, getRegistry, saveRegistry, generateId, HEX_size } from "../utils.js";
const { ApplicationV2, HandlebarsApplicationMixin } = foundry.applications.api;

export class HexFlowerManager extends HandlebarsApplicationMixin(ApplicationV2) {
...
export class HexFlowerEditor extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    async _prepareContext(options) {
        ...
        if (!this.editorState.draft) {
             ...
             if (this.flowerId && registry[this.flowerId]) {
                 ...
                 this.editorState.draft = {
                     ...
                     navigationRules: original.navigationRules ? JSON.parse(JSON.stringify(original.navigationRules)) : [],
        ...
        const directionOptions = { "N": "N", "NE": "NE", "SE": "SE", "S": "S", "SW": "SW", "NW": "NW", "SAME": "Stay" };
        const edgeBehaviorOptions = { "stop": "Stop", "wrap": "Wrap", "reflect": "Reflect", "loop": "Loop" };
        ...
        if (target.closest(".props-form")) {
            ...
            if (this.editorState.selectedHexIndex >= 0) {
                ...
                if (cell) {
                    ...
                    if (["color", "emoji", "title", "name"].includes(field)) this.render();
                }
            }
        } else if (target.closest(".tab[data-tab='rules']")) {
            // Rules or Globals
            if (target.classList.contains("rule-min") || target.classList.contains("rule-max") || target.classList.contains("rule-dir")) {
                const idx = parseInt(target.dataset.idx);
                const key = target.dataset.key;
                this.state.draft.navigationRules[idx][key] = (key === 'dir') ? val : parseInt(val);
            } else {
                 if (field === "edgeBehavior" || field === "partyActorId") {
                     this.state.draft[field] = val;
                 }
            }
        } else {
            // Main fields
            if (field === "name") this.state.draft.name = val;
        }
    }
    ...
    static async _onAddHex(event, target) {
        ...
        const cells = this.state.draft.data.cells;
        let base = { q: 0, r: 0 };
        if (cells.length > 0) {
            const idx = this.state.selectedHexIndex >= 0 ? this.state.selectedHexIndex : cells.length - 1;
        ...
        this.state.selectedHexIndex = cells.length - 1;
        this.render();
    }

    static async _onDeleteHex(event, target) {
        if (this.state.selectedHexIndex >= 0) {
            this.state.draft.data.cells.splice(this.state.selectedHexIndex, 1);
            this.state.selectedHexIndex = -1;
            this.render();
        }
    }

    static async _onAddRule(event, target) {
        this.state.draft.navigationRules.push({ min: 0, max: 0, dir: "SAME" });
        this.render();
    }

    static async _onDeleteRule(event, target) {
        const idx = parseInt(target.dataset.idx);
        this.state.draft.navigationRules.splice(idx, 1);
        this.render();
    }
    ...
    static async _onParseJson(event, target) {
        ...
        try {
            ...
            if (parsed.data || parsed.cells) {
                 if (parsed.data) this.state.draft.data = parsed.data;
                 if (parsed.cells) this.state.draft.data.cells = parsed.cells;
                 if (parsed.navigationRules) this.state.draft.navigationRules = parsed.navigationRules;
                 if (parsed.edgeBehavior) this.state.draft.edgeBehavior = parsed.edgeBehavior;
                 this.state.selectedHexIndex = -1;
                 this.render();
    ...
    static async _onCopyJson(event, target) {
         ...
         navigator.clipboard.writeText(json).then(() => ui.notifications.info("Copied!"));
    }
